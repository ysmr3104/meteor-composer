...
- **伸縮する制御は `minWidth` 0 を疑う。** `CheckBox` で一度踏んだのに、`ComboBox` で同じことをした。実測でツールバー 490 px のとき combo は 65 px、420 px で 42 px まで潰れる。最も広い項目「Unlinked」は文字だけで 47 px 必要なので、操作者が実際に開く幅で切れていた。**下限は `adjustToContents()` に申告させた値を使う**（自分で数字を決めない）。現在確認済み: `CheckBox`、`ComboBox`、`Label`

- **PJSR のダイアログにスクロールバーは無い。** 固定幅ペインの合計が行の幅を超えると、あふれた分はスクロールせず画面外へ出る。操作者からは「表示が左にずれていく」「枠が消えた」と見える。**固定幅の上限はウィンドウの実幅から計算すること**（`paneBudget()`）。一律の上限（900 だった）はウィンドウを見ていない
...
**単体テストは通っていた。** テスト側も同じ思い込みでインデックス配列を渡していたためである。捕まえたのは**正解データとの突き合わせだけ**だった。

**フィクスチャは実際の生産者から作ること。** 現在このテストは `connectedComponents` を呼んでその出力を渡す。表現が変われば落ちる。手で書いた配列は「自分がそう思っている形」でしかない。

同じ形の失敗をもう 1 件やっている。分類器のテストが理由文に `stationary` という語を含むかを見ていたため、操作者向けに書き直した時点で失敗した（守るべき性質は保たれていた）。**語ではなく意味を検査すること。**

## 実装した特徴量が実際に供給されているか確かめる

緑成分比は 6.1.0 で実測し（balanced accuracy 0.801、検出の主力である伸長比の 0.540 より強い）、分類器に `colourWeight 0.7` の項まで書いてあった。**`row.colour` を誰も設定していなかったので、一度も効いていなかった。**

同じ形の失敗が同時に 2 つ見つかっている。`mergeCollinear` も実装とテストがありながらどこからも呼ばれていなかった。**「実装した」と「効いている」は別。** 特徴量を足したら、実データで値が入っているかを数えて確かめること（`with colour 360 / 360` のように）。
...
## 実行時の不変条件は、読み方の規約に変換すれば静的に守れる

`this.displayed[this.currentRow]` を読む箇所が 6 つあり、**4 つは `currentRow < 0` しか見ていなかった。** フィルタでリストが選択行より短くなると範囲外を読んで落ち、一度落ちると以降の操作すべてで落ち続けた（操作者が `Show` を触って踏んだ）。

個々に上限チェックを足すのではなく、読み出しを **`currentDisplayedRow()` 1 か所に閉じ**、`tests/ut/test_module_isolation.js` が**直接読みを禁止する静的検査**を持つようにした。

`MeteorComposer.js` は構文解析しかできない（実行には PJSR のオブジェクトモデル全部が要る）ので、**「この形で読んではいけない」に落とせるものは落とすこと。** 実行時にしか現れない条件でも、読み方を 1 つに強制すれば静的に守れる。

## 永続化を単一の終了経路に賭けない

`saveSettings()` は `main()` の `dialog.execute()` の直後にしかなかった。操作者がマスクの傾きを 0 に戻して閉じ、再起動したら元の値が出た。**保存された 3 値（`listWidth 220` / `detailWidth 510` / 傾き 37）が「別のセッションが残した値」で揃っていた**ことから、その回は `saveSettings` が走っていないと判断できた。`execute()` から例外で抜ける、ダイアログではなくアプリを閉じる、といった経路で飛ばされる。

**このプロジェクトは判定について既に正しい判断をしている**（打つたびに自動保存する）。同じ理屈がマスクと表示設定にも当てはまる。変更したその場で保存し、終了時の保存は取りこぼしの受け皿として残す。

保存は**編集ごとのハンドラではなく、全編集が通る 1 か所から呼ぶ**（マスクなら `refreshMask()`）。制御を足して保存を忘れる経路を作れなくなる。

**復元中の抑止フラグは構築の最初に立てる。** メソッドの先頭では足りない。制御を組む段階で `refreshMask()` が走るので、フラグが下りていると復元より先に空の値を保存済みの値へ上書きし、**起動するたびに設定が消える。**

## 表示リストの位置は再構築を跨いで意味を持たない

フィルタとソートで作り直されるリストでは、**位置ではなく行の `id` で追うこと。** 411 件中の 40 行目と 92 件中の 40 行目は別の候補であり、存在しないこともある。位置で追うと、範囲外で落ちるか、別の候補に黙って飛ぶ。
...
両端の深さがどちらも 0 以上だと、境界線はその辺を横切れない。したがってバンドは必ず辺を端から端まで覆う。操作者が実際に必要としたマスク（境界が上辺の 25% 地点から抜け、左辺の 83% 地点から抜ける角の三角形）は、深さ+傾きなら `Left: 10%, tilt +24` だが、両端方式では左辺の下端側が **−5.1%** になり表現できない。**具体例を 1 つ座標で解けば 1 分で分かったことだった。**

同時に「半平面では辺の一部だけを覆えない」とも述べたが、これも誤り。半平面とフレームの交わりは、境界が隣り合う 2 辺を抜けるときは三角形になる。**自分がその挙動のテスト（`0% with a tilt is not a no-op`）を書いていたのに、そのことを忘れて逆の主張をした。**

**「A は B と等価」「A では原理的にできない」と書くときは、具体例を 1 つ数値で解いてから書くこと。** パラメータの形を変える判断は表現力の判断であり、表現力は検算できる。
...
## 1 つの数値で 2 つの故障を区別しようとしてはいけない

`fit.scale` が 1 から離れている、という 1 つの事実に原因が 2 つある。

| 傾き | レベル比 | 実際に起きていること |
|---|---|---|
| 0.10 | 0.11 | **本当に暗い。** 露出や ISO が違う。fit は信用できる |
| 0.10 | 0.97 | **明るさは同じで細部が噛み合っていない。** デベイヤー忘れ、登録失敗、追尾ぶれ。どの傾きも信用できない |

**傾きだけを見て文言を書くと必ず嘘になる。** 実際に「the frame is 10.3 times dimmer than the master」と書きかけたが、実測した median 比は 0.974 だった。操作者を露出設定の方へ誘導するところだった。

レベル比は `linearFit` が既に持っている和から無料で出る（`meanY / meanX`）。**2 つ揃って初めて診断になる。**

なお相関係数 r は使えない（実測: 正常な対でも 0.03〜0.07）。夜空は平坦で、単一サブの分散が自分のノイズと欠損領域に支配されるため。

## 範囲内であることは、値が正しいことを意味しない

上記の続き。欠損を除外した後、あるフレームの傾きは 0.310 で **0.2〜5.0 の範囲内**に収まった。レベル比は 0.9995。

この組で合成すると offset が平均を吸収するので、master が 0.5 を示す星の位置で fit の予測は 0.161、真値は 0.525、**残差 +0.36 をマスクが塗る。** 「範囲に入ったから通す」はこれを通してしまう。

**独立に測れる 2 つ目の量があるなら、範囲判定ではなくその 2 つの整合を条件にする。**

## 操作者の回避策を原因の証拠に使ってはいけない

マスクの位置がずれるという報告を受けて、テストディレクトリでマスクファイルを見つけてそのまま実測した。**そのファイルは操作者が既に画像エディタで回して調整した後のものだった。** 「経路は忠実」という結論は出たが、元のずれの原因は説明できない。**報告されたファイルを測る前に、それが報告時点のものかを確認すること。**
